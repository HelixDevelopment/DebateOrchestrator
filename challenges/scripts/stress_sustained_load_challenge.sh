...
# CONST-051(A). §11.4.85 sustained-load: N≥(DURATION×RPS) requests,
# p50 baseline vs post-load latency-degradation budget. No-target /
# unreachable → honest SKIP-OK (§11.4.3). Every PASS cites captured
# evidence (duration, request count, pass-pct, latency delta).
...
echo "=== DebateOrchestrator Stress Sustained-Load Challenge ==="
echo "  url=$HEALTH_URL dur=${DURATION}s rps=${RPS}"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/6] SKIP: DEBATEORCHESTRATOR_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== DebateOrchestrator Stress Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== DebateOrchestrator Stress Challenge: PASSED ==="
